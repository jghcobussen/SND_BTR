...
from utils_classification import DataPreprocess, Parameters, Parameters_reduced
from net_classification import SNDNet, MyDataset, digitize_signal
# usful module 
import torch
...
# selecting as many events as necessary to ensure equal numbers
event_limit_full = 2*event_limit
remove = int(len(reindex_TT_df)-event_limit_full)
reindex_TT_df = reindex_TT_df[:-remove]
reindex_y_full = reindex_y_full[:-remove]

# checking if there are equally many events of elastic and inelastic
...
#'''
# Save some useful information about the events in txt format 
...
np.save("elastic_train_Vi5.npy", a0)
np.save("inelastic_train_Vi5.npy", b0)
np.save("elastic_test_Vi5.npy", c0)
np.save("inelastic_test_Vi5.npy", d0)
...
np.save("True_Label_Vi5.npy",True_Label_test)
...
f=open("NN_performance_Label_Vi5.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 5 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_Label_Vi5.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 14, 19, 24, 29]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_Label_Vi5.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_Pred_Label_Vi5.npy",ans[:, 0])
