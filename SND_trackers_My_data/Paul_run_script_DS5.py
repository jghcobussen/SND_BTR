...
processed_file_path_2 = os.path.expandvars("/dcache/bfys/jcobus/ship_tt_processed_data")
#processed_file_path_2 = os.path.expandvars("/dcache/bfys/jcobus/DS5")
#processed_file_path_1 = os.path.expandvars("/dcache/bfys/jcobus/nue_NuEElastic/0to1k") #!!!!!!!!!!!!!!!!!!!!!CHANGE THE PATH !!!!!!!!!!!!!!!!
#processed_file_path_0 = os.path.expandvars("/dcache/bfys/jcobus/nue_CCDIS/0to1k")
...
chunklist_TT_df.append(pd.read_pickle(os.path.join(outpath_2, "tt_cleared_reduced.pkl"))) ### was tt_cleared_reduced
chunklist_y_full.append(pd.read_pickle(os.path.join(outpath_2, "y_cleared.pkl")))
...
for i in tqdm(range(n_steps-2)):  # tqdm: make your loops show a progress bar in terminal
    ...
    chunklist_TT_df.append(pd.read_pickle(os.path.join(outpath_2, "tt_cleared_reduced.pkl"))) ### was tt_cleared_reduced # add all the tt_cleared.pkl files read_pickle and add to the chunklist_TT_df list
    chunklist_y_full.append(pd.read_pickle(os.path.join(outpath_2, "y_cleared.pkl"))) # add all the y_cleared.pkl files read_pickle and add to the chunklist_y_full list
...
print("TT_df: " + str(len(reindex_TT_df)))
print("y_full: " + str(len(reindex_y_full)))

'''
for i in tqdm(range(n_steps)):
    j = i+10
    outpath_1 = processed_file_path_1 + "/{}".format(i)
    chunklist_TT_df.append(pd.read_pickle(os.path.join(outpath_1, "tt_cleared_reduced.pkl"))) ### was tt_cleared_reduced
...
#reindex_TT_df = reindex_TT_df.drop([270, 269, 268, 267, 266, 265, 264, 263, 262, 261, 260, 259, 258, 257, 256, 255, 254, 253, 252, 251, 250])
#reindex_y_full = reindex_y_full.drop([270, 269, 268, 267, 266, 265, 264, 263, 262, 261, 260, 259, 258, 257, 256, 255, 254, 253, 252, 251, 250])

#print("TT_df after drop(): " + str(len(reindex_TT_df)))
#print("y_full after drop(): " + str(len(reindex_y_full)))
...
True_E_test=y["E"][test_indeces]
np.save("True_E_DS5_mse_norm.npy",True_E_test)
...
f=open("NN_performance_DS5_mse_norm.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_DS5_mse_norm.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_DS5_mse_norm.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_PredE_L_norm.npy",ans[:, 0])
