...
from utils_full import DataPreprocess, Parameters, Parameters_reduced
...
f = open("train_test_indeces_Va.txt", "a")
...
np.save("elastic_train_V8.npy", a0)
np.save("inelastic_train_V8.npy", b0)
np.save("elastic_test_V8.npy", c0)
np.save("inelastic_test_V8.npy", d0)
...
np.save("True_Label_Va.npy",True_Label_test)
...
f=open("NN_performance_Label_Va.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_Label_Va.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29, 39]: #[9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_Label_Va.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_Pred_Label_Va.npy",ans[:, 0])
