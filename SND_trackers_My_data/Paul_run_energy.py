...
processed_file_path_2 = os.path.expandvars("/dcache/bfys/jcobus/nue_CCDIS/0to200k_new") #!!!!!!!!!!!!!!!!!!!!!CHANGE THE PATH !!!!!!!!!!!!!!!!
step_size = 5000    # size of a chunk
file_size = 140000  # size of the BigFile.root file
...
f.close()
NORM = 1. / 2000 ### normalizing for energy range 0-2000
y["E"] *= NORM
#y["Z"] *= -1
#y["THETA"] *= (180/np.pi)
...
np.save("TrueE_ccdis_res.npy",True_E_test)
...
f=open("NN_performance_ccdis_res.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_ccdis_res.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_ccdis_res.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_ccdis_res.npy",ans[:, 0])
