...
np.save("TrueE_test_norm1.npy",TrueE_test)
...
f=open("NN_performance_norm1.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_norm1.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_norm1.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save("PredE_file/" + str(i) + "_PredE_test_norm1.npy",ans[:, 0])
