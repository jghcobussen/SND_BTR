...
processed_file_path_1 = os.path.expandvars("/dcache/bfys/jcobus/nue_NuEElastic/0to200k") #!!!!!!!!!!!!!!!!!!!!!CHANGE THE PATH !!!!!!!!!!!!!!!!
processed_file_path_0 = os.path.expandvars("/dcache/bfys/jcobus/nue_CCDIS/0to200k/new")
...
chunklist_TT_df.append(pd.read_pickle(os.path.join(outpath_0, "tt_cleared_reduced.pkl"))) ### was tt_cleared_reduced
...
for i in tqdm(range(n_steps-2)):  # tqdm: make your loops show a progress bar in terminal
    ...
    chunklist_TT_df.append(pd.read_pickle(os.path.join(outpath_0, "tt_cleared_reduced.pkl"))) ### was tt_cleared_reduced # add all the tt_cleared.pkl files read_pickle and add to the chunklist_TT_df list
...
for i in tqdm(range(n_steps*1.9)):
    j = i+20
...
np.save("True_Label_V3.npy",True_Label_test)
...
f=open("NN_performance_Label_V3.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_Label_V3.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29]: #[9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_Label_dummy.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_Pred_Label_V3.npy",ans[:, 0])
