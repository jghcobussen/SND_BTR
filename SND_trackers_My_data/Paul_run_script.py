...
nb_of_planes = len(params.snd_params[params.configuration]["TT_POSITIONS"])
response = digitize_signal(reindex_TT_df.iloc[index], params=params, filters=nb_of_planes)
...
#plt.figure(figsize=(18,nb_of_planes))
for i in range(nb_of_planes):
    plt.subplot(1,nb_of_planes,i+1)
...
f = open("train_test_indeces_V9.txt", "a")
...
np.save("True_Label_V9.npy",True_Label_test)
...
f=open("NN_performance_Label_V9.txt","a")
...
#            for X_batch, y_batch in train_batch_gen:
            ...
            if (epoch + 1) % 10 == 0:
                with open("9X0_file/" + str(epoch) + "_9X0_coordconv_Label_V9.pt", 'wb') as f:
                    torch.save(net, f)       
                lr = lr / 2
...
for i in [9, 19, 29, 39, 49, 59]: #[9, 19, 29, 39]:
    net = torch.load("9X0_file/" + str(i) + "_9X0_coordconv_Label_V9.pt")
    preds = []
    with torch.no_grad():
        for (X_batch, y_batch) in test_batch_gen:
            preds.append(net.predict(X_batch))
    ans = np.concatenate([p.detach().cpu().numpy() for p in preds])
    np.save(str(i) + "_Pred_Label_V9.npy",ans[:, 0])
